...
class Price_matrix_model extends Ci_Model
{
        ...
        foreach ($flavour as $flav):
            ...
            foreach ($serving as $serv):
                ...
                if($price_matrix[$i] > 0 ){
                    ...
                    $this->db->set(array('location_id'=>$location_id,'cake_id'=> $flav,'serving_id'=> $serv,'price'=> $matrix_price))->insert('price_matrix');
                }
    ...
    public function getCakes()
    {
        ...
        return $this->db->select('*')->where(array('status'=> 1,'is_deleted !='=>1))->order_by("title", "asc")->limit(10)->get('cakes')->result();

    }

    public function getAll($location_id)
    {
        $data = $this->db->select('price_matrix_id,location_id,cake_id,serving_id,price')->where(array('location_id'=>$location_id,'price >'=>0,'is_deleted !='=>1))->order_by('price_matrix_id','asc')->get('price_matrix')->result_array();
